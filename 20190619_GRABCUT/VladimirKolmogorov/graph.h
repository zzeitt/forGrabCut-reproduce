...
class Graph {
 ...
 public:
  ...
  void setFlow(flowtype flow_arg);
 ...
 private:
  ...
  struct node {
    ...
    int is_sink : 1;    // flag showing whether the node is in the source or in
                        // the sink tree (if parent!=NULL)
...
template <typename captype, typename tcaptype, typename flowtype>
inline void Graph<captype, tcaptype, flowtype>::setFlow(flowtype flow_arg) {
  flow = flow_arg;
}
